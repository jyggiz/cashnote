...
import type { Promocode } from '../../src/types'
import { requireAuth } from './_auth'
import { readStore, writeStore } from './_store'

const STORE = 'promocodes'
...
export const handler: Handler = async (event, _context) => {
  ...
  try {
    if (event.httpMethod === 'GET') {
      requireAuth(event)
      const items = await readStore<Promocode>(STORE)
      return {
        statusCode: 200,
    ...
    if (event.httpMethod === 'POST') {
      requireAuth(event)
      const data = JSON.parse(event.body ?? '{}') as Omit<Promocode, 'id' | 'createdAt'>
      const items = await readStore<Promocode>(STORE)
      const newItem: Promocode = { ...data, id: randomUUID(), createdAt: new Date().toISOString() }
      items.push(newItem)
    ...
    if (event.httpMethod === 'PUT') {
      ...
      const data = JSON.parse(event.body ?? '{}') as Partial<Promocode>
      const items = await readStore<Promocode>(STORE)
